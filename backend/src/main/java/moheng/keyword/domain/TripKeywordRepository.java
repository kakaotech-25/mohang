...
public interface TripKeywordRepository extends JpaRepository<TripKeyword, Long> {
    @Query("SELECT tk FROM TripKeyword tk WHERE tk.keyword.id = :keywordIds")
    List<TripKeyword> findTripKeywordsByKeywordIds(@Param("keywordIds") final List<Long> keywordId);

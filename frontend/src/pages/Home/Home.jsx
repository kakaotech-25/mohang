  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        const keywords = keywordResponse.data.findAllKeywordResponses.map(
          (keyword) => ({
        ...
        const tripResponse = await axiosInstance.post(
          "/keyword/trip/recommend",
          { keywordIds: [] }
        );
              ...
              {uniqueKeywords.map((keyword, index) => (
                <button
                  ...
                  className={`keyword-button ${
                    selectedKeywords.includes(keyword.id) ? "selected" : ""
                  }`}
